...
class AnimateSlider extends StylePluginBase {
    ...
    public function buildOptionsForm(&$form, FormStateInterface $form_state) {
        ...
        for ($i = 0; $i < count($field_labels); $i++) {
            ...
            $form[$field_labels[$i]]['delay'] = array(
                '#type' => 'number',
                '#title' => $this->t('Delay'),
                '#attributes' => array(
                    'min' => 0,
                    ...
                    'value' => (isset($this->options[$field_labels[$i]]['delay'])) ?
                            $this->options[$field_labels[$i]]['delay'] : $this->options[$field_labels[$i]]['delay'],
                ),
                '#description' => t('Specifies delay in seconds.'),
            );
        }
    }
        ...
        for ($i = 0; $i < count($this->view->result); $i++) {
            for ($j = 0; $j < count($field_labels); $j++) {
                ...
                $field_type = $field_item_list->getFieldDefinition()->getType();
                    ...
                    $renderData[$i][$field_type] = array('value' => $this->view->style_plugin->getField($i, $field_labels[$j]), 'label' => $field_labels[$j]);
